...
const bili_client = {
     ...
     * - 源动态禁止转发 5
     */
    async autoRelay(uid, dyid, msg = '转发动态', ctrl = '[]', at_uids) {
        ...
        const
            responseText = await post({
                ...
                contents: {
                    ...
                    ctrl,
                    csrf: GlobalVar.get('csrf'),
                    ...(at_uids ? { at_uids } : {})  // 仅在 at_uids 存在时加入
                }
            }),
            res = strToJson(responseText);
        switch (res.code) {
     ...
     * - 验证码错误 12
     */
    async sendChat(rid, msg, type, at_users, code) {
        const
            responseText = await post({
                url: API.REPLY_ADD,
                contents: {
                    ...
                    type: type,
                    message: msg,
                    ...(at_users && { at_name_to_mid: JSON.stringify(at_users) }),
                    code,
                    csrf: GlobalVar.get('csrf')
                }
            }),
     ...
     * - 验证码错误 12
     */
    async sendChatWithOcr(rid, msg, type, at_users) {
     ...
     * 获取AT数据
     */
    async getAtInfoByNames(names) {
        const responseText = await get({
            url: API.X_POLYMER_WEB_DYNAMIC_V1_MENTION_SEARCH,
        });
        const res = strToJson(responseText);
        let atInfo = {};

        // 仅查找 group_type=2 的 items
        const targetItems = [];
        if (res.code === 0 && res.data?.groups) {
            const targetGroup = res.data.groups.find(group => group.group_type === 2);
            if (targetGroup?.items?.length) {
                targetItems.push(...targetGroup.items);
            }
        }

        // 遍历 names，仅返回有 uid 的匹配项
        names.forEach(name => {
            const foundItem = targetItems.find(item => item.name === name);
            if (foundItem?.uid) {
                atInfo[name] = foundItem.uid;
            }
        });
