...
class AIService {
    ...
    async getAnswer(question, options = {}) {
        ...
        try {
            ...
            const result = await providerModule.callAPI({
                question,
                options,
            ...
            console.error(`调用${this.selectedProvider.name} API出错:`, errorMessage);

            // 根据错误类型提供不同的错误信息
            if (errorMessage.includes('超时') || errorMessage.includes('timeout')) {
                throw new Error(`AI服务请求超时，请稍后重试。模型: ${this.selectedProvider.model}`);
            } else if (errorMessage.includes('API key') || errorMessage.includes('401')) {
                throw new Error(`AI服务认证失败，请检查API密钥配置。提供商: ${this.selectedProvider.name}`);
            } else if (errorMessage.includes('网络') || errorMessage.includes('连接')) {
                throw new Error(`网络连接失败，请检查网络连接和baseUrl配置。baseUrl: ${this.selectedProvider.baseUrl}`);
            } else {
