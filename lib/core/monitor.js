        ...
        if (rid && chat_type) {
            ...
            if (process.env.IS_AI_CHAT) {
                try {
                    ...
                    log.info('AI的回答', ai_answer)
                    const usernames = ai_answer.match(/@([^@\s]+)/g).map(name => name.slice(1));
                    ...
                    if (is_repost_then_chat) {
                        ...
                        Object.entries(at_users).forEach(([name, uid]) => {
                            // 查找 @name 在 content 中的位置
                            const atPattern = new RegExp(`@${name}`, "g");
                            let match;

                            while ((match = atPattern.exec(content)) !== null) {
                                const location = match.index; // @ 符号的位置
                                const length = name.length + 1; // @ + 用户名长度
                        ...
                        ctrl = JSON.stringify(ctrl_arr)
                        at_uids = Object.values(at_users).join(",");
                    }
                } catch (error) {
