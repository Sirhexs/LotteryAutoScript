...
class Monitor extends Searcher {
    ...
    async filterLotteryInfo() {
        ...
        await try_for_each(protoLotteryInfo, async function (lottery_info) {
            ...
            if (isLottery) {
                ...
                onelotteryinfo.isRelayDynamic = isRelayDynamic;
                ...
                if (isSendChat) {
                    ...
                    onelotteryinfo.chat = (getRandomOne(chats) || '!!!')
                        .replace(/\$\{uname\}/g, uname);
                    onelotteryinfo.description = description;
                }
    ...
    async go(option) {
        ...
        if (rid && chat_type) {
            ...
            // 接入ai评论
            // log.info('接入deepseek获取评论',description)
            if (process.env.IS_AI_CHAT) {
                try {
                    const ai_answer = await ai.getAnswer(description, {
                        systemPrompt: process.env.AI_SYSTEM_PROMPT || "充当一个参加动态抽奖的评论专家，如果动态中明确了需要进行@或者带上话题，你需确保评论满足要求，并且把评论控制在50字以内，不需要给我评论的说明，直接给我评论内容即可。",
                    });
                    log.info('AI生成的评论内容', ai_answer)
                    chat = ai_answer

                    // 安全提取评论中的@用户名（如果有的话）
                    const mentionMatches = ai_answer ? ai_answer.match(/@([^@\s]+)/g) : null;

                    if (mentionMatches && mentionMatches.length > 0) {
                        try {
                            // 去除@符号获取纯用户名
                            const usernames = mentionMatches.map(name => name.slice(1));
                            // 获取用户UID信息
                            at_users = await bili.getAtInfoByNames(usernames) || {};

                            log.info('成功获取@用户信息', at_users);
                        } catch (e) {
                            log.warn('获取@用户信息失败，继续执行', e);
                        }
                    }

                    if (is_repost_then_chat && !isRelayDynamic) {
                        relay_chat = chat;
                        let ctrl_arr = []
                        // 只有当存在@用户时才处理
                        if (at_users && Object.keys(at_users).length > 0) {
                            Object.entries(at_users).forEach(([name, uid]) => {
                                const atPattern = new RegExp(`@${name}`, "g");
                                let match;

                                // 查找所有@位置
                                while ((match = atPattern.exec(chat)) !== null) {
                                    ctrl_arr.push({
                                        location: match.index,  // @位置
                                        type: 1,               // 控制类型
                                        length: name.length + 1, // @符号+用户名长度
                                        data: uid,            // 用户UID
                                    });
                                }
                            });
                        ...
                        // 序列化控制信息
                        ctrl = ctrl_arr.length > 0 ? JSON.stringify(ctrl_arr) : "[]";
                        // 拼接所有@用户的UID
                        at_uids = ctrl_arr.length > 0 ? Object.values(at_users).filter(Boolean).join(",") : null
                    }
                } catch (error) {
                    log.error("AI服务出错", error);
                    log.info('调用ai获取评论失败，使用随机回答', chat)
                }
            }
